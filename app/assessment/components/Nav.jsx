...
const Navbar = () => {
  ...
  return (
    <header>
      {/******** White background section with logo and contact information ********/}
          ...
          <div className="flex items-center space-x-2">
            <div className="hidden md:flex items-center space-x-2 text-sm text-black uppercase font-semibold italic">
              {["Bangalore", "Hyderabad", "USA"].map((location) => (
                <div
                  key={location}
                  className="flex items-center animate-pulse text-custom-blue"
                >
      ...
      <div className="bg-orange-500 fixed top-12 w-screen z-50 md:hidden">
        ...
        {/******** Mobile Menu ********/}
        <div
          className={`absolute top-12 left-0 w-full bg-orange-500 transition-all ease-in-out duration-300 ${isMenuOpen ? "block" : "hidden"}`}
        >
          {menuItems.map((item) => (
            <Link
              key={item.name}
              href={item.path}
              className="block px-4 py-2 text-white text-sm font-semibold hover:bg-white hover:bg-opacity-20 uppercase"
